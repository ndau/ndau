...
func generateAccount(t *testing.T, balance math.Ndau, hasLock, hasStake bool) (AccountData, string) {
	...
	ad := AccountData{
		...
		Holds:              []Hold{},
		RecourseSettings:   generateEscrowSettings(randBool()),
	...
	for i := 0; i < qtyEscrows; i++ {
		ad.Holds = append(ad.Holds, generateEscrow())
	}
	if randBool() {
...
func generateEscrowSettings(changing bool) RecourseSettings {
	es := RecourseSettings{
		Period: randDuration(),
	}
	if changing {
...
func TestUpdateEscrow(t *testing.T) {
	...
	require.Equal(t, qtyEscrows/2, len(acct.Holds))
