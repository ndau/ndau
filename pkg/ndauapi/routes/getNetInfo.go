...
func GetNetInfo(cf cfg.Cfg) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			reqres.RespondJSON(w, reqres.NewError(fmt.Sprintf("Could not fetch net info: %v", err), http.StatusInternalServerError))
			return
		}
		reqres.RespondJSON(w, reqres.Response{Sts: http.StatusOK, Bd: netInfo})
	}
}
